...
library(leaflet)
library(dplyr)
fname <- normalizePath("filecfc22271009.sqlite")
random_id <- function() {
...
dummytab <- data_frame(ID = 0, name = "dummy", lead = "dummy", 
                       date_start = Sys.Date(), date_end = Sys.Date(), team_size = 0L, show = FALSE)
...
get_data <- function() {  
  ...
  rm(db)
  tab$date_start <- tab$date_start + epoch
  tab$date_end <- tab$date_end + epoch
  tab
}
